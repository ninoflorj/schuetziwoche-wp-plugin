...
function schuetziwoche_field_input($config, $field, $checked = false) {
	...
	$disabled = schuetziwoche_is_disabled($config, $field);
	$expired = time() > $config['date'][$day] + $limit;
	return '<input type="checkbox" name="' . $field . '" value="1"' . ($checked && !$disabled ? ' checked="checked"' : '') . ($disabled || $expired ? ' disabled="disabled"' : '') . '>';
}
...
function schuetziwoche_install() {
	...
	$sql = "CREATE TABLE $table_name (
		...
		mo_payed smallint(1) default '0',
		di_payed smallint(1) default '0',
		mi_payed smallint(1) default '0',
		do_payed smallint(1) default '0',
		fr_payed smallint(1) default '0',
		PRIMARY KEY  (id)
