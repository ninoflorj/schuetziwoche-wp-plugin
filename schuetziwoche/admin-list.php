...
class TT_Example_List_Table extends WP_List_Table {
    ...
    function __construct(){
        ...
        parent::__construct( array(
            'singular'  => 'Anmeldung',     //singular name of the listed records
            'plural'    => 'Anmeldungen',    //plural name of the listed records
    ...
    function column_name($item){
        ...
        $actions = array(
            ...
            'delete'    => sprintf('<a href="?page=%s&action=%s&id=%s" onclick="return confirm(\'Diese Anmeldung wirklich loeschen?\');">Delete</a>',$_REQUEST['page'],'delete',$item['id']),
        );
    ...
    function column_cb($item){
        return sprintf(
            '<input type="checkbox" name="%1$s[]" value="%2$s" form="movies-filter" />',
    ...
    function get_sortable_columns() {
        $sortable_columns = array(
            ...
            'abteilung'    => array('abteilung',false),
            'email'  => array('email',false),
            'isvegi'  => array('isvegi',false),
    ...
    function process_bulk_action() {
        ...
        if( 'delete'===$this->current_action() ) {
            ...
            if (isset($_REQUEST['id']) && is_numeric($_REQUEST['id'])){
                $wpdb->query($wpdb->prepare("DELETE FROM ".$config['table']." WHERE id = %s", $_REQUEST['id']));
                echo '<div class="updated below-h2"><p>Anmeldung mit der ID '.$_REQUEST['id'].' geloescht!</p></div>';
            } else {
                $in = "('".implode("','", $_REQUEST['anmeldung'])."')";
                $wpdb->query("DELETE FROM ".$config['table']." WHERE id IN ".$in);
                echo '<div class="updated below-h2"><p>'.count($_REQUEST['anmeldung']).' Anmeldungen geloescht!</p></div>';
            }
        }        
    }
    ...
    function prepare_items() {
        ...
        */
        ...
        $data = $wpdb->get_results("SELECT * FROM ".$config['table']." ORDER BY ".$orderby." ".$order."", ARRAY_A);
